...
export function useFinance() {
  const [categories, setCategories] = useState<Awaited<ReturnType<typeof window.yumiV2.finance.listCategories>>>([])
  const [advancePayers, setAdvancePayers] = useState<Awaited<ReturnType<typeof window.yumiV2.finance.listAdvancePayers>>>([])
  const [entries, setEntries] = useState<Awaited<ReturnType<typeof window.yumiV2.finance.listEntries>>>([])
  const [pendingReimbursements, setPendingReimbursements] = useState<Awaited<ReturnType<typeof window.yumiV2.finance.listPendingReimbursements>>>([])
  ...
  const reload = useCallback(async (entryQuery?: V2FinanceEntryQuery) => {
    ...
    try {
      const [nextCategories, nextPayers, nextEntries, nextPending] = await Promise.all([
        ...
        window.yumiV2.finance.listPendingReimbursements()
      ])
  ...
  const createCategory = useCallback(async (input: V2FinanceCategoryCreateInput) => {
    const category = await window.yumiV2.finance.createCategory(input)
    await reload()
    return category
  }, [reload])
  const updateCategory = useCallback(async (id: string, input: V2FinanceCategoryUpdateInput) => {
  ...
  const createAdvancePayer = useCallback(async (input: V2AdvancePayerCreateInput) => {
    const payer = await window.yumiV2.finance.createAdvancePayer(input)
    await reload()
    return payer
  }, [reload])
  const updateAdvancePayer = useCallback(async (id: string, input: V2AdvancePayerUpdateInput) => {
    const payer = await window.yumiV2.finance.updateAdvancePayer(id, input)
    await reload()
    return payer
  }, [reload])
  const deleteAdvancePayer = useCallback(async (id: string) => {
    await window.yumiV2.finance.deleteAdvancePayer(id)
    await reload()
  }, [reload])
  const createManualIncome = useCallback(async (input: V2ManualIncomeInput) => {
    const entry = await window.yumiV2.finance.createManualIncome(input)
    await reload()
    return entry
  }, [reload])
  const createManualExpense = useCallback(async (input: V2ManualExpenseInput) => {
    const entry = await window.yumiV2.finance.createManualExpense(input)
    await reload()
    return entry
  }, [reload])
  const reimburse = useCallback(async (input: V2ReimbursementInput) => {
  ...
  const linkAfterSalesCharge = useCallback(async (afterSalesCaseId: string, financialEntryId: string) => window.yumiV2.afterSales.linkCharge(afterSalesCaseId, financialEntryId), [])

  return {
    categories, advancePayers, entries, pendingReimbursements, loading, loadError, reload,
