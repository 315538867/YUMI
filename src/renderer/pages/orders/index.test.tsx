...
describe('订单详情概览信息架构', () => {
  it('概览只保留订单主体、全宽关键指标、商品表和排班概览，不重复发货记录', async () => {
    ...
    const scheduleOverview = screen.getByRole('region', { name: '订单排班概览' })
    expect(within(scheduleOverview).getByText('待制作')).toBeVisible()
    expect(within(scheduleOverview).getByText('待捏毛装袋')).toBeVisible()
    expect(within(scheduleOverview).getByText('待打包')).toBeVisible()
    expect(within(scheduleOverview).getByText('待发货')).toBeVisible()
