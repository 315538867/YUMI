...
import type { V2WorkerSettlementDetail } from '@shared/contracts'
...
export function SettlementsPage() {
  ...
  return <div className="settlements-workspace">
    <div className="page-heading"><div><Text size="2" color="gray">兼职工资</Text><Heading size="7">工资结算</Heading></div><Badge color="orange">负责人确认实发</Badge></div>
    {loadError && <div className="panel"><Text color="red">{loadError}</Text></div>}
    {error && <div className="panel"><Text color="red">{error}</Text></div>}
    <WorkersPage workers={workers} createWorker={createWorker} listWageHistory={listWageHistory} recordWageHistory={recordWageHistory} />
    <div className="two-column">
      <form className="panel editor-form" onSubmit={handleCreateDraft}>
        <Heading size="4">新建结算草稿</Heading>
