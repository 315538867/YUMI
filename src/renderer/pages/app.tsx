...
export function App() {
  ...
  const activeItem = useMemo(
    () => navigationGroups.flatMap((group) => group.items).find((item) => item.id === view),
    [view]
  )
  const openNavigationTarget = (target: V2NavigationTarget) =>
    setNavigation({ view: target.view, target })
    ...
    <div className="yumi-app-shell">
        ...
        <nav aria-label="主导航" className="yumi-app-navigation">
          {navigationGroups.map((group) => (
            <section
              ...
              className="yumi-app-navigation__group"
              key={group.label}
            >
              ...
              {group.items.map((item) => {
                ...
                return (
                  <YumiButton
                    ...
                    className={
                      active
                        ? 'yumi-app-navigation__item yumi-app-navigation__item--active'
                        : 'yumi-app-navigation__item'
                    }
                    key={item.id}
      ...
      <main className="yumi-app-workspace">
        <header className="yumi-app-command-bar">
          <div>
            <span>YUMI 捏捏工作室</span>
            <strong>{activeItem?.label}</strong>
          </div>
          {navigation.target ? (
            <YumiButton onClick={returnToWorkbench} variant="ghost">
              返回工作台
            </YumiButton>
          ) : (
            <span>负责人工作台</span>
        ...
        <div className="yumi-app-content">
          ...
          {view === 'orders' && (
            <OrdersPage
              ...
              onNavigate={openNavigationTarget}
              onNavigateToBaseData={selectNavigation}
            />
          )}
          {view === 'fulfillment' && (
            <FulfillmentPage
              navigationTarget={
                navigation.target?.view === 'fulfillment' ? navigation.target : null
              }
              onNavigate={openNavigationTarget}
            />
          )}
          {view === 'settlements' && (
            <SettlementsPage
              navigationTarget={
                navigation.target?.view === 'settlements' ? navigation.target : null
              }
            />
          )}
          {view === 'finance' && (
          ...
          {view === 'products' && (
            <ProductsPage
              navigationTarget={navigation.target?.view === 'products' ? navigation.target : null}
            />
          )}
