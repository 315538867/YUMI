...
export function ReportsPage({ onNavigate }: ReportsPageProps) {
  ...
  return (
    <div className="yumi-page yumi-report-page">
      <YumiPageHeader
        description="只读取已确认的订单、履约、收付款和工资事实；风险记录只提供进入实际处理区的入口。"
        title="经营报表"
      />
      <YumiSection description="按统计月份查看实际收付款、经营支出和已确认工资。" title="月度经营">
        ...
        <div className="yumi-finance-summary-grid">
          ...
          <Metric
            label="经营支出"
            cents={monthlyOperation?.operatingExpenseCents ?? 0}
            tone="danger"
          />
          <Metric
            label="经营结果"
            cents={monthlyOperation?.operatingResultCents ?? 0}
            tone={(monthlyOperation?.operatingResultCents ?? 0) >= 0 ? 'success' : 'danger'}
          />
          <Metric
            label="已确认工资"
            cents={monthlyOperation?.confirmedSettlementPaidCents ?? 0}
            tone="brand"
          />
        </div>
      </YumiSection>
      ...
      <YumiSection
        description="以制作截止日为准识别已逾期、临近交期和未计划的未完成订单；处理需进入履约工作区。"
        title="交期风险"
      >
        <div className="yumi-section-toolbar">
          <YumiField>
      ...
      <YumiSection
        description="阶段数量来自期初在制品、质检合格、打包和发货等履约事件。"
        title="履约进度"
      >
        <div className="yumi-section-inline-status">
...
function Metric({
  ...
  return (
    <article className={`yumi-finance-metric yumi-finance-metric--${tone}`}>
      <span>{label}</span>
