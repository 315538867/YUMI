...
export function ReportsPage({ onNavigate }: ReportsPageProps) {
  const { orderBusiness, fulfillmentProgress, confirmedSettlements, monthlyOperation, capacityRisk, deliveryRisk, loading, loadError, exporting, exportMessage, load, exportCurrentReport, exportOrderTable, exportShippingList } = useReports()
  ...
  const exportOrders = useCallback(async () => {
    try { setError(null); await exportOrderTable() } catch (cause) { setError(getErrorMessage(cause)) }
  }, [exportOrderTable])
  const exportShipping = useCallback(async () => {
    try { setError(null); await exportShippingList() } catch (cause) { setError(getErrorMessage(cause)) }
  }, [exportShippingList])
  const filteredDeliveryRows = useMemo(() => (deliveryRisk?.rows ?? []).filter((row) => riskFilter === 'all' || row.level === riskFilter), [deliveryRisk, riskFilter])

  return <div className="yumi-page yumi-report-page">
    <YumiPageHeader description="只读取已确认的订单、履约、收付款和工资事实；风险记录只提供进入实际处理区的入口。" title="经营报表" />
    {(loadError || error || exportMessage) && <YumiNotification key={loadError ?? error ?? exportMessage} message={loadError ?? error ?? exportMessage!} />}

    <YumiSection description="按统计月份查看实际收付款、经营支出和已确认工资。" title="月度经营">
      <div className="yumi-section-toolbar">
        ...
        <YumiButton disabled={loading} onClick={() => void reload()} variant="secondary">刷新</YumiButton>
        <YumiButton disabled={loading || exporting} onClick={() => void exportReport()} variant="primary">导出当前报表</YumiButton>
        <YumiButton disabled={loading || exporting} onClick={() => void exportOrders()} variant="secondary">导出订单表</YumiButton>
        <YumiButton disabled={loading || exporting} onClick={() => void exportShipping()} variant="secondary">导出发货清单</YumiButton>
      </div>
    ...
    <YumiSection description="周期内待制作需求、已排制作量与商品模具日产能并列展示。点击商品只查看基础资料；修改参数需在资料页主动编辑。" title="商品产能风险">
      <div className="yumi-section-toolbar">
        <YumiField><YumiFieldLabel>开始日期</YumiFieldLabel><YumiDatePicker aria-label="产能风险开始日期" onValueChange={setCapacityStartOn} value={capacityStartOn} /></YumiField>
        <YumiField><YumiFieldLabel>结束日期</YumiFieldLabel><YumiDatePicker aria-label="产能风险结束日期" onValueChange={setCapacityEndOn} value={capacityEndOn} /></YumiField>
        <YumiButton disabled={loading} onClick={() => void reload()} variant="secondary">刷新风险</YumiButton>
      </div>
      <YumiDataTable<V2CapacityRiskReportRow>
        columns={[
          { key: 'product', label: '商品', render: (row) => <strong>{row.productName}</strong> },
          { align: 'right', key: 'demand', label: '待制作需求', render: (row) => `${row.demandQuantity} 件` },
          { align: 'right', key: 'scheduled', label: '已排制作', render: (row) => `${row.scheduledQuantity} 件` },
          { align: 'right', key: 'capacity', label: '可用产能', render: (row) => `${row.availableCapacityQuantity} 件` },
          { align: 'right', key: 'gap', label: '缺口', render: (row) => `${row.gapQuantity} 件` },
          { key: 'risk', label: '风险', render: (row) => <><RiskTag level={row.level} /><br /><span className="yumi-table-secondary">{row.riskSources.join('；') || '暂无'}</span></> },
          { key: 'action', label: '处理', render: (row) => <YumiButton onClick={() => onNavigate?.({ view: 'products', productId: row.productRoute.productId })} variant="ghost">查看商品</YumiButton> }
        ]}
    ...
    <YumiSection description="以制作截止日为准识别已逾期、临近交期和未计划的未完成订单；处理需进入履约工作区。" title="交期风险">
      <div className="yumi-section-toolbar">
        <YumiField><YumiFieldLabel>截至日期</YumiFieldLabel><YumiDatePicker aria-label="交期风险截至日期" onValueChange={setDeliveryAsOf} value={deliveryAsOf} /></YumiField>
        <YumiField><YumiFieldLabel>风险级别</YumiFieldLabel><YumiSelect aria-label="交期风险级别" onValueChange={(value) => setRiskFilter(value as RiskFilter)} options={[{ label: '全部级别', value: 'all' }, { label: '严重', value: 'critical' }, { label: '预警', value: 'warning' }, { label: '未计划', value: 'unplanned' }, { label: '正常', value: 'normal' }]} value={riskFilter} /></YumiField>
        <YumiButton disabled={loading} onClick={() => void reload()} variant="secondary">刷新风险</YumiButton>
      </div>
      <YumiDataTable<V2DeliveryRiskReportRow>
        columns={[
          { key: 'order', label: '订单', render: (row) => <strong>{row.orderCode}</strong> },
          { key: 'expected', label: '预计发货', render: (row) => row.expectedShipDate ?? '未填写' },
          { key: 'deadline', label: '制作截止', render: (row) => row.productionDeadline ?? '未计划' },
          { align: 'right', key: 'remaining', label: '待发数量', render: (row) => `${row.remainingQuantity} 件` },
          { key: 'risk', label: '风险', render: (row) => <><RiskTag level={row.level} /><br /><span className="yumi-table-secondary">{row.riskSources.join('；') || '暂无'}</span></> },
          { key: 'action', label: '处理', render: (row) => <YumiButton onClick={() => onNavigate?.({ view: 'fulfillment', orderId: row.fulfillmentRoute.orderId, focus: 'queue' })} variant="primary">进入履约处理</YumiButton> }
        ]}
    ...
    <YumiSection description="成本为商品下单快照成本与售后核算成本，不将未建立分摊规则的工资强行计入订单。" title="订单核算">
      <YumiDataTable<V2OrderBusinessReportRow>
        columns={[
          ...
          { align: 'right', key: 'current', label: '订单金额', render: (row) => formatCents(row.currentAmountCents) },
...
function RiskTag({ level }: { level: V2RiskLevel }) {
  const labels: Record<V2RiskLevel, string> = { normal: '正常', warning: '预警', critical: '严重', unplanned: '未计划' }
  const tones: Record<V2RiskLevel, 'neutral' | 'warning' | 'danger' | 'success'> = { normal: 'success', warning: 'warning', critical: 'danger', unplanned: 'neutral' }
