...
export function CustomersPage() {
  ...
  const customerList = <div className="panel">
    <Heading size="4">客户列表</Heading>
    {loadError && <p className="form-error">{loadError}</p>}
    {loading ? <p className="empty">正在加载客户…</p> : <div className="data-list">
      {customers.map((customer) => <button type="button" className="data-list-row" key={customer.id} onClick={() => openEdit(customer)}>
        <span><strong>{customer.name}</strong><small>{customer.contact || '未填写联系人'} · {customer.defaultAddress || '未填写默认地址'}</small></span>
  ...
  return <section className="v2-page">
    <Flex justify="between" align="center" gap="4" className="page-title-row">
      <div><Heading size="6">客户</Heading><Text as="p" color="gray">维护订单可关联的客户资料；订单会保留当时的客户快照。</Text></div>
      <Button onClick={openCreate}>新建客户</Button>
    </Flex>
    {editorOpen && <div className="v2-two-column">
      {customerList}
      <form className="panel v2-form" onSubmit={submit}>
        <Flex justify="between" align="center"><Heading size="4">{editing ? `编辑客户：${editing.name}` : '新建客户'}</Heading><Button type="button" variant="ghost" color="gray" onClick={closeEditor}>关闭</Button></Flex>
        ...
        <Flex gap="3" justify="end"><Button type="button" variant="soft" color="gray" onClick={closeEditor}>取消</Button><Button type="submit" disabled={submitting}>{submitting ? '保存中…' : editing ? '保存客户' : '创建客户'}</Button></Flex>
      </form>
    </div>}
    {!editorOpen && customerList}
  </section>
}
