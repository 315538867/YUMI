...
const mocks = vi.hoisted(() => ({
  ...
  state: {
    categories: [], advancePayers: [], entries: [], pendingReimbursements: [],
    monthlySummary: { incomeCents: 50_000, operatingExpenseCents: 20_000, operatingResultCents: 30_000 },
    loading: false, loadError: null,
    createManualIncome: vi.fn(), createManualExpense: vi.fn()
  }
}))

vi.mock('../../composables/use-finance', () => ({ useFinance: () => ({ ...mocks.state, loadMonthlyOverview: mocks.loadMonthlyOverview, reimburseBatch: mocks.reimburseBatch }) }))
...
afterEach(() => {
  ...
  mocks.state.monthlySummary = { incomeCents: 50_000, operatingExpenseCents: 20_000, operatingResultCents: 30_000 }
})
...
describe('财务负责人工作区', () => {
  ...
  it('负责人可在同一待报销列表选择多笔后，以单次确认原子提交', async () => {
    mocks.state.pendingReimbursements = [
      { financialEntryId: 'advance-1', amountCents: 10_000, occurredOn: '2026-09-02', categoryId: 'expense-1', categoryName: '包装材料', advancePayerId: 'payer-1', advancePayerName: '小林', note: '第一笔' },
      { financialEntryId: 'advance-2', amountCents: 20_000, occurredOn: '2026-09-03', categoryId: 'expense-1', categoryName: '包装材料', advancePayerId: 'payer-1', advancePayerName: '小林', note: '第二笔' }
    ]
    render(<FinancePage />)
    ...
    expect(sheet).toHaveTextContent('本次将报销 2 笔私人垫付，合计 ¥300.00。任一记录已被报销或无效时，本次不会产生部分报销。')
    fireEvent.change(screen.getByRole('textbox', { name: '报销支付方式' }), { target: { value: '公账转账' } })
