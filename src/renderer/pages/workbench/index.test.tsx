...
const mocks = vi.hoisted(() => ({
  ...
  state: {
    ...
    snapshot: {
      ...
      firstUseGuide: null,
      decisionItems: [{
        id: 'quality-inspection:task-1', kind: 'quality_inspection' as const, bucket: 'decision' as const, priority: 'urgent' as const,
        subject: { title: '确认质检结果', description: '制作 · 排班 2026-09-08' }, quantityOrAmount: { kind: 'quantity' as const, value: 8, unit: '件' },
        dueHint: '完成后待质检 · 2026-09-08', navigationTarget: { view: 'fulfillment' as const, processTaskId: 'task-1', focus: 'inspection' as const }
      }],
      advanceItems: [{
        id: 'shipment:item-1', kind: 'shipment' as const, bucket: 'advance' as const, priority: 'urgent' as const,
        subject: { title: '登记发货', description: 'YD-001 · 草莓团子' }, quantityOrAmount: { kind: 'quantity' as const, value: 3, unit: '件' },
        dueHint: '预计发货 2026-09-10', navigationTarget: { view: 'fulfillment' as const, orderItemId: 'item-1', focus: 'shipment' as const }
      }]
    }
  }
}))
...
describe('负责人工作台页面', () => {
  it('在两个互斥视图中只呈现一个主任务列表，并将事项交给真实处理区', () => {
    ...
    expect(onNavigate).toHaveBeenCalledWith({ view: 'fulfillment', orderItemId: 'item-1', focus: 'shipment' })
  })
