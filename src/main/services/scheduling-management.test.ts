...
describe('排班时长与完成数量', () => {
  ...
  it('按任务基础时长与本次全局额外时长计算最终总时长', () => {
    ...
    expect(context.service.getShiftDetail(shift.id)?.tasks[0]).toMatchObject({
      baseMinutes: 29,
      estimatedMinutes: 29
    })
  })
  ...
  it('标记已完成时必须完整填写每项任务的合格与不合格数量，并原子保存', () => {
    ...
    expect(() =>
      context.service.updateShiftStatus({ shiftId: shift.id, status: 'completed' })
    ...
    ).toEqual(['ORDER_QUANTITY_EXCEEDED'])
  })

  it('一次排班只允许安排一个订单内的一种商品', () => {
    const context = createService()
    databases.push(context.database)
    const product = context.service.createProduct({ ...productInput, standardMinutesPerUnit: 10 })
    const worker = context.service.createWorker({ name: '小林', hourlyWageCents: 2800 })
    ...
    expect(() =>
      context.service.previewShift({
        ...
        tasks: [
          ...
          { orderItemId: order.items[1]!.id, plannedQuantity: 1 }
        ]
      })
    ).toThrow('一次排班只能安排一个订单内的一种商品')
  })
