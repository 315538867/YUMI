...
describe('AfterSalesService', () => {
  ...
  it('从原发货建立售后时保留批次上下文，负责人显式确认只保存售后事实，后续履约和收费仍须另行创建', () => {
    ...
    fulfillment.recordOpeningWip({
      orderItemId, targetStage: 'ready_to_ship', quantity: 2,
      occurredOn: '2026-09-06', note: '系统启用前已完成打包'
    })
    const originalShipment = orderService.createShipment(order.id, {
      shippedOn: '2026-09-06', carrier: '顺丰', trackingNo: 'SF-ORIGINAL-001',
      items: [{ orderItemId, quantity: 2 }]
    })

    const caseRecord = afterSales.createCase({
      orderId: order.id, shipmentId: originalShipment.id, occurredOn: '2026-09-08',
      reasonDescription: '客户认为包装袋封边不足', customerRequest: '加封边并更换包装袋',
      responsibilityDescription: '负责人确认由工作室承担重新包装成本',
      handlingDescription: '负责人确认后另行安排重新包装', status: 'processing',
      customerChargeNote: '本次不向客户收费', accountingCostCents: 260, note: '少量免费处理'
    })

    expect(caseRecord).toMatchObject({
      orderId: order.id, shipmentId: originalShipment.id, status: 'processing',
      responsibilityDescription: '负责人确认由工作室承担重新包装成本'
    })
    ...
    expect(() => afterSales.createCase({
      ...
      reasonDescription: '不应关联其他订单的发货批次', customerRequest: null,
      responsibilityDescription: '负责人判断', handlingDescription: '负责人处理', status: 'open',
      customerChargeNote: null, accountingCostCents: 0, note: null
    ...
    expect(database.prepare('SELECT COUNT(*) AS count FROM financial_entries').get()).toEqual({ count: 0 })
    expect(fulfillment.getOrderItemFulfillment(orderItemId).stages).toMatchObject({ making: 1, readyToShip: 0, shipped: 2 })
  })
