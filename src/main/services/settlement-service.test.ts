...
describe('SettlementService', () => {
  ...
  it('分别汇总制作、捏毛装袋、打包和发货任务，参考工资只按单一口径计算且捏毛不合格不扣胶水', () => {
    ...
    const fulfillmentService = new FulfillmentService(new V2FulfillmentRepository(database))
    const settlementService = new SettlementService(database)
    const worker = settlementService.createWorker({ name: '小夏', hourlyWageCents: 2_000, effectiveOn: '2026-09-01' })
    const product = orderService.createProduct({
    ...
    const makingResult = fulfillmentService.submitProcessResult(making.tasks[0].id, { completedQuantity: 2, submittedOn: '2026-09-07' })
    fulfillmentService.confirmQualityInspection(makingResult.id, { qualifiedQuantity: 1, unqualifiedQuantity: 1, inspectedOn: '2026-09-07' })

    const rework = fulfillmentService.createWorkAssignment({
      workerId: worker.id, assignedOn: '2026-09-07', processType: 'making',
      tasks: [{ orderItemId, sourceType: 'rework', plannedQuantity: 1 }]
    })
    const reworkResult = fulfillmentService.submitProcessResult(rework.tasks[0].id, { completedQuantity: 1, submittedOn: '2026-09-07' })
    fulfillmentService.confirmQualityInspection(reworkResult.id, { qualifiedQuantity: 1, unqualifiedQuantity: 0, inspectedOn: '2026-09-07' })

    const fluffing = fulfillmentService.createWorkAssignment({
      workerId: worker.id, assignedOn: '2026-09-07', processType: 'fluffing_bagging',
      tasks: [{ orderItemId, sourceType: 'normal_production', plannedQuantity: 2, plannedMinutes: 20, pieceRateCents: 100 }]
    })
    const fluffingResult = fulfillmentService.submitProcessResult(fluffing.tasks[0].id, { completedQuantity: 2, submittedOn: '2026-09-07' })
    fulfillmentService.confirmQualityInspection(fluffingResult.id, { qualifiedQuantity: 1, unqualifiedQuantity: 1, inspectedOn: '2026-09-07' })

    const packing = fulfillmentService.createWorkAssignment({
      workerId: worker.id, assignedOn: '2026-09-07', processType: 'packing',
      tasks: [{ orderItemId, sourceType: 'normal_production', plannedQuantity: 1, plannedMinutes: 10 }]
    })
    fulfillmentService.submitProcessResult(packing.tasks[0].id, { completedQuantity: 1, submittedOn: '2026-09-07' })

    const shipping = fulfillmentService.createWorkAssignment({
      workerId: worker.id, assignedOn: '2026-09-07', processType: 'shipping',
      tasks: [{ orderItemId, sourceType: 'normal_production', plannedQuantity: 1, plannedMinutes: 15 }]
    })
    fulfillmentService.submitProcessResult(shipping.tasks[0].id, { completedQuantity: 1, submittedOn: '2026-09-07' })

    const draft = settlementService.createDraft({
      workerId: worker.id, periodStartOn: '2026-09-07', periodEndOn: '2026-09-07', attendanceMinutes: 90
    })
  ...
  it('已确认工资在后续发现制作不合格时保留历史实发并转为待退款，负责人可登记退款结果', () => {
    ...
    fulfillmentService.confirmQualityInspection(makingResult.id, {
      qualifiedQuantity: 3, unqualifiedQuantity: 0, inspectedOn: '2026-09-08'
    })
    const settled = settlementService.createDraft({
      workerId: worker.id, periodStartOn: '2026-09-07', periodEndOn: '2026-09-08'
    })
    settlementService.updateDraft(settled.id, {
      finalPaidAmountCents: 1_800, paidOn: '2026-09-09', managerNote: '已实际发放'
    })
    ...
    const shippingAssignment = fulfillmentService.createWorkAssignment({
      workerId: worker.id, assignedOn: '2026-09-10', processType: 'shipping',
      tasks: [{ orderItemId: order.items[0].id, sourceType: 'normal_production', plannedQuantity: 1, plannedMinutes: 30 }]
    })
    fulfillmentService.submitProcessResult(shippingAssignment.tasks[0].id, {
      completedQuantity: 1, submittedOn: '2026-09-10'
    })
    settlementService.createDraft({
      workerId: worker.id, periodStartOn: '2026-09-10', periodEndOn: '2026-09-10'
    })
