...
describe('FinanceService', () => {
  it('登记私人垫付并整笔报销：保留现金付款事实但不重复计入经营支出', () => {
    ...
    `
    ...
    ).toEqual([
      {
        ...
        direction: 'expense',
        business_type: 'daily_expense',
        amount_cents: 12_500,
        payment_source: 'private_advance',
        advance_payer_id: payer.id
      },
      {
        source_type: 'reimbursement',
        direction: 'expense',
        business_type: 'advance_reimbursement',
        amount_cents: 12_500,
        payment_source: 'business_account',
        advance_payer_id: null
      }
    ])
    expect(database.prepare('SELECT COUNT(*) AS count FROM advance_reimbursements').get()).toEqual({
      count: 1
    })
    ...
    expect(
      database
        .prepare(
          "SELECT COUNT(*) AS count FROM audit_logs WHERE action = 'finance.reimbursement_created'"
        )
        .get()
    ).toEqual({ count: 1 })
  })
  ...
  it('批量报销会先完整校验，成功时分别建流水，任何无效选择均不产生部分报销', () => {
    ...
    const reimbursementCount = database
      .prepare(
        "SELECT COUNT(*) AS count FROM financial_entries WHERE source_type = 'reimbursement'"
      )
      .get()
    expect(() =>
      service.reimburseBatch({
        advanceFinancialEntryIds: [firstAdvance.id, firstAdvance.id],
        reimbursedOn: '2026-09-06'
      })
    ).toThrow('不能重复选择同一私人垫付')
    expect(() =>
      service.reimburseBatch({
        advanceFinancialEntryIds: [businessExpense.id, secondAdvance.id],
        reimbursedOn: '2026-09-06'
      })
    ).toThrow('报销对象必须是私人垫付日常支出')
    expect(() =>
      service.reimburseBatch({
        advanceFinancialEntryIds: [firstAdvance.id, businessExpense.id],
        reimbursedOn: '2026-09-06'
      })
