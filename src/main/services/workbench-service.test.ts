...
function assignment(
  taskId = 'task-1',
  overrides: Partial<V2WorkAssignment['tasks'][number]> = {}
): V2WorkAssignment {
  return {
    ...
    tasks: [
      {
        id: taskId,
        workAssignmentId: `assignment-${taskId}`,
        orderItemId: 'item-1',
        processType: 'making',
...
function order(): V2Order {
  return {
    ...
    funds: { receivedCents: 0, refundedCents: 0, netReceivedCents: 0, outstandingCents: 10_000 },
...
function afterSales(): V2AfterSalesCase {
  return {
    ...
    reasonDescription: '包装不满意',
    customerRequest: '加封边',
    responsibilityDescription: '待负责人确认',
    handlingDescription: '待商议',
    status: 'open',
    customerChargeNote: null,
    accountingCostCents: 300,
    note: null,
...
function refund(): V2WorkerRefundRecord {
  return {
    id: 'refund-1',
    workerId: 'worker-1',
    originalSettlementId: 'settlement-confirmed-1',
    processTaskId: 'task-refund-1',
    processResultId: 'result-refund-1',
    qualityInspectionId: 'inspection-refund-1',
    orderId: 'order-1',
    orderItemId: 'item-1',
...
function createService(
  ...
  return new WorkbenchService({
    orders: {
      listCustomers: () =>
        Array.from({ length: input.customers ?? 1 }, (_, index) => ({ id: `customer-${index}` })),
      listProducts: () =>
        Array.from({ length: input.products ?? 1 }, (_, index) => ({ id: `product-${index}` })),
      listOrders: () =>
        input.orders ?? [
          {
            id: 'order-1',
...
describe('WorkbenchService', () => {
  ...
  it('无待办时仅返回当前最先缺失的首用前置操作，读取不会写入领域状态', () => {
    ...
    const productGuide = createService({
      customers: 1,
      products: 0,
      orders: [],
      assignments: [],
