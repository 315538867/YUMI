...
describe('V2 财务与售后验收', () => {
  it('以动态资料、实际日期、私人垫付和负责人显式售后动作形成可追溯闭环', () => {
    ...
    expect(finance.listCategories('income', true)).toEqual(
    ...
    const beforeFreeCaseCashCount = Number(
      (
        database.prepare('SELECT COUNT(*) AS count FROM financial_entries').get() as {
          count: number
        }
      ).count
    )
    const freeCase = afterSales.createCase({
      ...
      reasonDescription: '客户希望更换包装袋',
      customerRequest: '换袋并加封边',
      responsibilityDescription: '负责人决定少量免费处理',
      handlingDescription: '重新包装',
      status: 'processing',
      customerChargeNote: '本次不收费',
    ...
    const paidCase = afterSales.createCase({
      ...
      reasonDescription: '客户追加加封边',
      customerRequest: '加封边',
      responsibilityDescription: '客户定制需求变更',
      handlingDescription: '负责人确认后处理',
      status: 'resolved',
    ...
    expect(afterSales.listCases({ orderId: order.id })).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: freeCase.id,
          accountingCostCents: 260,
    ...
    expect(finance.getMonthlySummary('2026-09')).toEqual({
      incomeCents: 1_300,
      operatingExpenseCents: 500,
      operatingResultCents: 800
    })
    expect(finance.listEntries({ fromOn: '2026-09-01', toOn: '2026-09-30' })).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: advance.id,
          paymentSource: 'private_advance',
