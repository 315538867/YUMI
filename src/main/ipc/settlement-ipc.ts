...
export function registerSettlementIpc(ipc: SettlementIpcMain, service: SettlementService): void {
  ...
  ipc.handle('v2:workers:wages:list', (_event, workerId) =>
    service.listWageHistory(workerId as string)
  )
  ipc.handle('v2:workers:wages:record', (_event, input) =>
    service.recordWageHistory(input as never)
  )
  ...
  ipc.handle('v2:settlements:get', (_event, settlementId) =>
    service.getSettlement(settlementId as string)
  )
