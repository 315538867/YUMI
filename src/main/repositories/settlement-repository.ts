...
function mapSettlementTaskSource(row: Record<string, unknown>): SettlementTaskSource {
  return {
    ...
    processType: row.process_type as SettlementTaskSource['processType'],
    ...
    productSnapshotJson: row.product_snapshot_json as string | null,
    ...
    inspectedOn: row.inspected_on as string | null
  }
}
...
function mapSettlement(row: Record<string, unknown>): V2WorkerSettlement {
  return {
    ...
    attendanceMinutes: row.attendance_minutes === null ? null : Number(row.attendance_minutes),
...
function mapSettlementTask(row: Record<string, unknown>): V2WorkerSettlementTask {
  return {
    id: String(row.id),
    processTaskId: String(row.process_task_id),
    processType: row.process_type as V2WorkerSettlementTask['processType'],
    pieceRateCents: row.piece_rate_cents === null ? null : Number(row.piece_rate_cents),
    scheduledMinutes: Number(row.scheduled_minutes),
    qualifiedQuantity: Number(row.qualified_quantity),
    qualifiedCommissionCents: Number(row.qualified_commission_cents),
    status: row.status as V2WorkerSettlementTask['status'],
    createdAt: String(row.created_at)
  }
}
...
function mapDeduction(row: Record<string, unknown>): V2WorkerDeductionRecord {
  return {
    ...
    processTaskId: String(row.process_task_id),
    processType: row.process_type as V2WorkerDeductionRecord['processType'],
    pieceRateCents: row.piece_rate_cents === null ? null : Number(row.piece_rate_cents),
    processResultId: row.process_result_id as string | null,
    qualityInspectionId: row.quality_inspection_id as string | null,
    orderId: row.order_id as string | null,
    orderItemId: row.order_item_id as string | null,
    unqualifiedQuantity: Number(row.unqualified_quantity),
    commissionDeductionCents: Number(row.commission_deduction_cents),
    wageDeductionCents: Number(row.wage_deduction_cents),
    glueDeductionCents: Number(row.glue_deduction_cents),
...
function mapRefund(row: Record<string, unknown>): V2WorkerRefundRecord {
  return {
    ...
    wageDeductionCents: Number(row.wage_deduction_cents),
    glueDeductionCents: Number(row.glue_deduction_cents),
    requestedRefundCents: Number(row.requested_refund_cents),
...
export class SettlementRepository {
  ...
  listEligibleTaskSources(
    workerId: string,
    periodStartOn: string,
    periodEndOn: string
  ): SettlementTaskSource[] {
    const rows = this.database
      .prepare(
        `SELECT
  ...
  insertDeduction(record: V2WorkerDeductionRecord): void {
    this.database
      ...
      .run(
        ...
        record.commissionDeductionCents,
        record.wageDeductionCents,
        record.glueDeductionCents,
  ...
  insertSettlement(settlement: V2WorkerSettlement): void {
    this.database
      ...
      .run(
        ...
        settlement.attendanceReferenceWageCents,
  ...
  listSettlementTasks(settlementId: string): V2WorkerSettlementTask[] {
    return (
      this.database
        .prepare(
          `SELECT settlement_tasks.*, process_tasks.process_type, process_tasks.piece_rate_cents
       FROM worker_settlement_tasks settlement_tasks
       JOIN process_tasks ON process_tasks.id = settlement_tasks.process_task_id
       WHERE settlement_tasks.settlement_id = ?
       ORDER BY settlement_tasks.created_at ASC, settlement_tasks.id ASC`
        )
        .all(settlementId) as Array<Record<string, unknown>>
    ).map(mapSettlementTask)
  }
  ...
  listSettlementDeductions(settlementId: string): V2WorkerDeductionRecord[] {
    return (
      this.database
        .prepare(
          `SELECT deductions.*, quality_inspections.inspected_on, process_tasks.process_type, process_tasks.piece_rate_cents
       FROM worker_deduction_records deductions
       JOIN worker_settlement_deduction_allocations allocations ON allocations.deduction_record_id = deductions.id
       JOIN process_tasks ON process_tasks.id = deductions.process_task_id
       LEFT JOIN quality_inspections ON quality_inspections.id = deductions.quality_inspection_id
  ...
  confirmSettlementArtifacts(settlementId: string, now: string): void {
    ...
    this.database
      .prepare(
        "UPDATE worker_settlement_tasks SET status = 'confirmed' WHERE settlement_id = ? AND status = 'draft'"
      )
      .run(settlementId)
    this.database
      .prepare(
