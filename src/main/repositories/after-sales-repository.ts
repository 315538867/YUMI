...
function mapCase(row: Record<string, unknown>): V2AfterSalesCase {
  return {
    id: String(row.id), orderId: String(row.order_id), shipmentId: row.shipment_id as string | null,
    occurredOn: String(row.occurred_on), reasonDescription: String(row.reason_description),
    customerRequest: row.customer_request as string | null, responsibilityDescription: String(row.responsibility_description),
    handlingDescription: String(row.handling_description), status: row.status as V2AfterSalesCase['status'],
    customerChargeNote: row.customer_charge_note as string | null, accountingCostCents: Number(row.accounting_cost_cents),
    note: row.note as string | null, chargeFinancialEntryIds: parseChargeIds(row.charge_financial_entry_ids),
...
export class AfterSalesRepository {
  ...
  shipmentBelongsToOrder(shipmentId: string, orderId: string): boolean {
    return Boolean(this.database.prepare('SELECT 1 FROM shipments WHERE id = ? AND order_id = ?').get(shipmentId, orderId))
  }
  ...
  insertCase(input: AfterSalesCaseWrite): void {
    this.database.prepare(
      ...
      input.id, input.orderId, input.shipmentId, input.occurredOn, input.reasonDescription, input.customerRequest,
      input.responsibilityDescription, input.handlingDescription, input.status, input.customerChargeNote,
      input.accountingCostCents, input.note, input.createdAt, input.updatedAt
    )
  }
  ...
  updateCase(input: AfterSalesCaseWrite): void {
    this.database.prepare(
      ...
      input.shipmentId, input.occurredOn, input.reasonDescription, input.customerRequest, input.responsibilityDescription,
      input.handlingDescription, input.status, input.customerChargeNote, input.accountingCostCents, input.note,
  ...
  getFinancialEntryForLink(id: string): {
    ...
    if (!row) return null
    return {
      id: String(row.id), orderId: row.order_id as string | null, sourceType: String(row.source_type),
      direction: String(row.direction), businessType: String(row.business_type)
    }
  }
