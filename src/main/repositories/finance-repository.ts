...
function mapCategory(row: Record<string, unknown>): V2FinanceCategory {
  return {
    id: String(row.id), direction: row.direction as V2FinanceDirection, name: String(row.name), enabled: Boolean(row.enabled),
...
function mapEntry(row: Record<string, unknown>): V2FinancialEntry {
  return {
    id: String(row.id), sourceType: row.source_type as V2FinancialEntry['sourceType'], direction: row.direction as V2FinanceDirection,
    businessType: String(row.business_type), amountCents: Number(row.amount_cents), occurredOn: String(row.occurred_on),
    paymentMethod: row.payment_method as string | null, paymentSource: row.payment_source as V2FinancialEntry['paymentSource'],
    categoryId: row.category_id as string | null, categoryName: row.category_name as string | null,
    advancePayerId: row.advance_payer_id as string | null, advancePayerName: row.advance_payer_name as string | null,
    orderId: row.order_id as string | null, attachmentId: row.attachment_id as string | null,
    reversalOfEntryId: row.reversal_of_entry_id as string | null, note: row.note as string | null, createdAt: String(row.created_at)
  }
}
...
export class FinanceRepository {
  ...
  listCategories(direction?: V2FinanceDirection, includeDisabled = false): V2FinanceCategory[] {
    ...
    ).all(direction ?? null, direction ?? null, includeDisabled ? 1 : 0) as Array<Record<string, unknown>>).map(mapCategory)
  }

  getCategory(id: string): V2FinanceCategory | null {
    const row = this.database.prepare('SELECT * FROM finance_categories WHERE id = ?').get(id) as Record<string, unknown> | undefined
  ...
  insertCategory(category: V2FinanceCategory): void {
    this.database.prepare(
      `INSERT INTO finance_categories (id, direction, name, enabled, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?)`
    ).run(category.id, category.direction, category.name, Number(category.enabled), category.createdAt, category.updatedAt)
  }

  updateCategory(category: V2FinanceCategory): void {
    this.database.prepare(
      'UPDATE finance_categories SET name = ?, enabled = ?, updated_at = ? WHERE id = ?'
    ).run(category.name, Number(category.enabled), category.updatedAt, category.id)
  }

  isCategoryReferenced(id: string): boolean {
    return Boolean(this.database.prepare('SELECT 1 FROM financial_entries WHERE category_id = ? LIMIT 1').get(id))
  }
  ...
  listAdvancePayers(includeDisabled = false): V2AdvancePayer[] {
    ...
    ).all(includeDisabled ? 1 : 0) as Array<Record<string, unknown>>).map(mapPayer)
  }

  getAdvancePayer(id: string): V2AdvancePayer | null {
    const row = this.database.prepare('SELECT * FROM advance_payers WHERE id = ?').get(id) as Record<string, unknown> | undefined
  ...
  insertAdvancePayer(payer: V2AdvancePayer): void {
    this.database.prepare(
      `INSERT INTO advance_payers (id, name, enabled, note, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?)`
    ).run(payer.id, payer.name, Number(payer.enabled), payer.note, payer.createdAt, payer.updatedAt)
  }
  ...
  isAdvancePayerReferenced(id: string): boolean {
    return Boolean(this.database.prepare('SELECT 1 FROM financial_entries WHERE advance_payer_id = ? LIMIT 1').get(id))
  }
  ...
  listFinancialEntries(query: V2FinanceEntryQuery = {}): V2FinancialEntry[] {
    const rows = this.database.prepare(
      ...
      query.direction ?? null, query.direction ?? null, query.sourceType ?? null, query.sourceType ?? null,
      query.fromOn ?? null, query.fromOn ?? null, query.toOn ?? null, query.toOn ?? null
  ...
  insertFinancialEntry(entry: FinanceEntryInsert): void {
    this.database.prepare(
      ...
      entry.id, entry.sourceType, entry.direction, entry.businessType, entry.amountCents, entry.occurredOn,
      entry.paymentMethod, entry.paymentSource, entry.categoryId, entry.advancePayerId, entry.orderId,
      entry.attachmentId, entry.reversalOfEntryId, entry.note, entry.createdAt
    )
  }

  getReimbursementForAdvance(advanceFinancialEntryId: string): { reimbursementFinancialEntryId: string; reimbursedOn: string } | null {
    const row = this.database.prepare(
    ...
    ).get(advanceFinancialEntryId) as { reimbursement_financial_entry_id: string; occurred_on: string } | undefined
    return row ? { reimbursementFinancialEntryId: row.reimbursement_financial_entry_id, reimbursedOn: row.occurred_on } : null
  }
  ...
  listReimbursementReferences(): Array<{ advanceFinancialEntryId: string; reimbursedOn: string }> {
    return this.database.prepare(
      ...
      advanceFinancialEntryId: String((row as Record<string, unknown>).advance_financial_entry_id),
      reimbursedOn: String((row as Record<string, unknown>).occurred_on)
    }))
  }

  insertReimbursementLink(id: string, advanceFinancialEntryId: string, reimbursementFinancialEntryId: string, createdAt: string): void {
    this.database.prepare(
  ...
  getAuditPayload(id: string): unknown | null {
    const row = this.database.prepare('SELECT after_json FROM audit_logs WHERE id = ?').get(id) as { after_json: string | null } | undefined
