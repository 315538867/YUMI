...
describe('排班风险预览', () => {
    ...
    const result = previewShiftRisks({
      ...
      tasks: [
        ...
        },
        {
          ...
          dueDate: '2026-09-20',
          dailyCapacity: 20,
          otherPlannedQuantityForDay: 0
        }
      ]
    })
    expect(result.taskBaseMinutes).toEqual([
      { orderItemId: 'oi1', baseMinutes: 29 },
      { orderItemId: 'oi2', baseMinutes: 45 }
    ])
  ...
  it('拒绝非整数或负数的全局预留时长', () => {
    ...
    expect(() => previewShiftRisks({ ...input, extraMinutes: -1 })).toThrow(
      '额外预留时长必须为非负整数'
    )
    expect(() => previewShiftRisks({ ...input, extraMinutes: 1.5 })).toThrow(
      '额外预留时长必须为非负整数'
    )
  })
})
