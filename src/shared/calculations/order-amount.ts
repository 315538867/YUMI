...
export interface OrderAmountCalculation {
  ...
  itemAndEdge: MoneyCalculationResult
  itemDiscount: MoneyCalculationResult
  orderDiscount: MoneyCalculationResult
  orderAmount: MoneyCalculationResult
}

export const orderAmountFormulaIds = {
  itemAmount: 'order.item_amount',
  edgeAmount: 'order.edge_amount',
  lineAmount: 'order.line_amount',
  itemAndEdgeSubtotal: 'order.item_and_edge_subtotal',
...
/**
 * 生成订单草稿金额的公式展示：商品与缝边小计、优惠与预计订单金额
 * 全部来自同一份订单金额汇总，页面不得自行重复求和。
 */
export function createOrderAmountCalculation(
  input: OrderAmountSummaryInput
): OrderAmountCalculation {
  const summary = calculateOrderAmountSummary(input)
  const itemAmountNode = variableNode({
    key: 'itemAmount',
    label: '商品金额合计',
    unit: 'cents',
    value: summary.itemAmountCents
  })
  const edgeAmountNode = variableNode({
    key: 'edgeAmount',
    label: '缝边金额合计',
    unit: 'cents',
    value: summary.edgeAmountCents
  })
  const itemDiscountNode = variableNode({
    key: 'itemDiscount',
    label: '明细优惠合计',
    unit: 'cents',
    value: summary.itemDiscountCents
  })
  const orderDiscountNode = variableNode({
    key: 'orderDiscount',
    label: '订单优惠',
    unit: 'cents',
    value: summary.orderDiscountCents
  })
  const subtotalNode = groupNode('商品与缝边小计', sumNode([itemAmountNode, edgeAmountNode]))

  return {
    summary,
    itemAndEdge: createMoneyCalculationResult({
      formulaId: orderAmountFormulaIds.itemAndEdgeSubtotal,
      formulaVersion: 1,
      label: '商品与缝边小计',
      node: subtotalNode
    }),
    itemDiscount: createMoneyCalculationResult({
      formulaId: orderAmountFormulaIds.itemDiscount,
      formulaVersion: 1,
      label: '明细优惠',
      node: itemDiscountNode
    }),
    orderDiscount: createMoneyCalculationResult({
      formulaId: orderAmountFormulaIds.orderDiscount,
      formulaVersion: 1,
      label: '订单优惠',
      node: orderDiscountNode
    }),
    orderAmount: createMoneyCalculationResult({
      formulaId: orderAmountFormulaIds.orderAmount,
      formulaVersion: 1,
      label: '预计订单金额',
      node: subtractNode(subtractNode(subtotalNode, itemDiscountNode), orderDiscountNode)
    })
  }
}
